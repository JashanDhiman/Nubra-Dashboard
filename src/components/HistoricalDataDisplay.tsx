  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-mono uppercase tracking-wider text-text-muted">Symbol</div>
                <div className="text-lg font-mono font-semibold text-foreground">{symbol}</div>
              </div>
            </div>
            <div className="w-px h-8 bg-border" />
            <div>
              <div className="text-xs font-mono uppercase tracking-wider text-text-muted">Exchange</div>
              <div className="text-sm font-mono font-medium text-foreground">{result.exchange}</div>
            </div>
            <div className="w-px h-8 bg-border" />
            <div>
              <div className="text-xs font-mono uppercase tracking-wider text-text-muted">Type</div>
              <div className="text-sm font-mono font-medium text-foreground">{result.type}</div>
            </div>
          </div>
          <div className="text-right">
            <div className="text-xs font-mono uppercase tracking-wider text-text-muted">Fields</div>
            <div className="text-sm font-mono font-medium text-foreground">{Object.keys(symbolData).length}</div>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        {Object.entries(symbolData).map(([field, points]: [string, HistoricalDataPoint[]]) => (
          <div key={field} className="bg-surface-2 rounded-lg border border-border overflow-hidden">
            {/* Field Header */}
            <div className="px-6 py-4 border-b border-border bg-gradient-to-r from-surface-3 to-surface-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full bg-accent-green" />
                  <h3 className="text-base font-semibold text-foreground font-mono capitalize">{field}</h3>
                  <span className="px-2 py-1 bg-accent-muted/20 text-accent-green text-xs font-mono rounded">
                    {points.length} points
                  </span>
                </div>
                <div className="text-xs font-mono text-text-muted">
                  Latest: {points.length > 0 ? formatTimestamp(points[points.length - 1].ts) : 'N/A'}
                </div>
              </div>
            </div>
              ...
              <div className="max-h-96 overflow-y-auto">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-border">
                    {points.slice(0, 20).map((point: HistoricalDataPoint, index: number) => (
                      <tr
                        key={index}
                  ...
                  <div className="px-6 py-4 bg-surface-3 border-t border-border">
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-text-muted font-mono">
                        Showing first 20 of {points.length.toLocaleString()} data points
                      </p>
                      <div className="flex items-center gap-4 text-xs font-mono text-text-muted">
                        <span>Range: {formatTimestamp(points[0].ts)} - {formatTimestamp(points[points.length - 1].ts)}</span>
                      </div>
                    </div>
                  </div>
                )}
