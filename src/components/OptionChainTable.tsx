...
function ChangeCell({ pct }: { pct: number }) {
  const isUp = pct >= 0;
  return (
    <span className={clsx("text-[11px] font-mono", isUp ? "text-accent-green" : "text-accent-red")}>
...
// ─── Greeks cells ─────────────────────────────────────────────────────────────
function GreekCell({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <td className="px-2 py-1.5 text-center">
      <span className="text-[10px] font-mono" style={{ color: color || "#7a8fa6" }}>
        {value.toFixed(2)}
      </span>
    </td>
  );
...
function DepthRow({ leg, side }: { leg: OptionLeg; side: "call" | "put" }) {
  ...
  return (
    <tr className="bg-surface-2/50">
      <td colSpan={side === "call" ? 8 : 0} className={clsx(side === "put" && "hidden")}>
        {side === "call" && (
          <div className="px-3 py-1">
            <div className="text-[9px] font-mono text-text-muted mb-0.5">TOP 3 BIDS</div>
            {topBids.map((b, i) => (
              <div key={i} className="flex gap-2 text-[10px] font-mono text-accent-green">
      ...
      <td colSpan={side === "put" ? 8 : 0} className={clsx(side === "call" && "hidden")}>
        {side === "put" && (
          <div className="px-3 py-1">
            <div className="text-[9px] font-mono text-text-muted mb-0.5">TOP 3 ASKS</div>
            {topAsks.map((a, i) => (
              <div key={i} className="flex gap-2 text-[10px] font-mono text-accent-red">
...
export function OptionChainTable() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <p className="text-text-muted font-mono text-sm">Loading option chain…</p>
        </div>
      </div>
    );
  }
  ...
  if (filteredRows.length === 0) {
    return (
      <div className="flex-1 flex items-center justify-center py-24">
        <p className="text-text-muted font-mono text-sm">No data. Select an expiry.</p>
      </div>
    );
  }
      ...
      <table className="w-full border-collapse text-[11px]" style={{ minWidth: 900 }}>
        <thead className="sticky top-0 z-10">
          <tr className="bg-surface-1 border-b border-border">
            {/* CALLS header */}
            <th colSpan={filter.showGreeks ? 8 : 5} className="py-1.5 px-2 text-center text-[9px] font-mono font-medium tracking-widest text-accent-blue uppercase border-r border-border bg-accent-blue/5">
              ← CALLS
            </th>
            ...
            {/* PUTS header */}
            <th colSpan={filter.showGreeks ? 8 : 5} className="py-1.5 px-2 text-center text-[9px] font-mono font-medium tracking-widest text-accent-red uppercase border-l border-border bg-accent-red/5">
              PUTS →
            </th>
          </tr>
          <tr className="bg-surface-2 border-b border-border text-[9px] font-mono text-text-muted uppercase tracking-widest">
            ...
            <th className="py-1.5 px-2 text-right bg-accent-blue/5 border-r border-border">Ask</th>

            {/* Strike */}
            <th className="py-1.5 px-3 text-center font-medium text-text-secondary">—</th>

            {/* Put columns */}
            <th className="py-1.5 px-2 text-left bg-accent-red/5 border-l border-border">Bid</th>
  ...
  return (
    <>
      ...
      >
        ...
        {/* Call Bid */}
        <td className="px-2 py-1.5 text-right bg-accent-blue/[0.02]">
          <PriceCell value={c.bid} token={c.instrument_token + "_bid"} flashState={flashState} />
        </td>
        ...
        >
          <PriceCell value={c.ltp} token={c.instrument_token} flashState={flashState} bold />
        </td>

        {/* Call Ask */}
        <td className="px-2 py-1.5 text-right bg-accent-blue/[0.02] border-r border-border">
          <PriceCell value={c.ask} token={c.instrument_token + "_ask"} flashState={flashState} />
        </td>
        ...
        <td className="px-2 py-1.5 text-left bg-accent-red/[0.02] border-l border-border">
          <PriceCell value={p.bid} token={p.instrument_token + "_bid"} flashState={flashState} />
        </td>
        ...
        <td
          ...
          onClick={onSelectPut}
        >
          <PriceCell value={p.ltp} token={p.instrument_token} flashState={flashState} bold />
        </td>

        {/* Put Ask */}
        <td className="px-2 py-1.5 text-left bg-accent-red/[0.02]">
          <PriceCell value={p.ask} token={p.instrument_token + "_ask"} flashState={flashState} />
        </td>
